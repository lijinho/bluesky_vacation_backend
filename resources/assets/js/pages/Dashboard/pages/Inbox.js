...
import {
  hideChatBoxes,
  showChatBoxes
} from "../../../actions/chatmodule/chatmoduleActions";
...
class Inbox extends React.Component {
  constructor(props) {
    ...
    this.notifySoundUrl =
      "https://res.cloudinary.com/vacation-rentals/video/upload/v1554130121/audio/alarm.mp3";
  ...
  archiveContact(contact_id) {
    Axios.post("/ajax/chatcontact/updatestatus", {
      contactId: contact_id,
      status: "archived",
      userId: metas["LogedUserId"].content
    }).then(result => {
      this.setState({
  ...
  componentDidMount() {
    if (this.state.isLogedIn == "true") {
      Axios.get(
        "/ajax/chat/getcontactlists?userId=" + metas["LogedUserId"].content
      ).then(result => {
        this.setState({
          contacts: result.data
        });
    ...
    return (
      <div className="inbox_chat">
        ...
        {this.state.active_contact ? (
            ...
            if (contact.id == this.state.active_contact) {
              return (
                <MessagePanel
                  ...
                  contactUser={contact}
                  my_id={metas["LogedUserId"].content}
                  my_profile_pic={metas["userProfilePic"].content}
                  user_id={contact.id}
          ...
          <MessagePanel
            openContact={() => this.openContactList()}
            isActive={this.state.active_contact == this.state.contacts[0].id}
            onTyping={() => this.activeChatBox(this.state.contacts[0].id)}
            contactUser={this.state.contacts[0]}
            my_id={metas["LogedUserId"].content}
            my_profile_pic={metas["userProfilePic"].content}
            user_id={this.state.contacts[0].id}
